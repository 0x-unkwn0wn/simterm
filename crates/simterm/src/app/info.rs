    ...
    fn help_post(lang: Language) -> Vec<String> {
        let src: &[&str] = match lang {
            Language::Es => &[
                ...
                "  cat <ruta>           - lee un fichero (lore, credenciales, botín)",
                "  exfil <ruta>         - exfiltra el fichero objetivo y completa el nivel",
                ...
                "  >> objetivo: exfiltra con 'exfil' el fichero objetivo para completar el nivel",
            ],
            Language::En => &[
                "[POST] (after getting a shell - explore the filesystem)",
                "  ls [path]            - list a directory",
                "  cd [path] / pwd      - change directory / show current directory",
                "  cat <path>           - read a file (lore, credentials, loot)",
                "  exfil <path>         - exfiltrate the objective file and complete the level",
                ...
                "  >> objective: exfiltrate the objective file with 'exfil' to complete the level",
            ],
        };
        src.iter().map(|s| s.to_string()).collect()
    }
